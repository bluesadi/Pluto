...
static cl::opt<InliningAdvisorMode>
    UseInlineAdvisor("enable-ml-inliner", cl::init(InliningAdvisorMode::Default), cl::Hidden,
...
static cl::opt<bool> EnableSyntheticCounts("enable-npm-synthetic-counts", cl::init(false),
                                           cl::Hidden, cl::ZeroOrMore,
                                           cl::desc("Run synthetic function entry count generation "
                                                    "pass"));

static const Regex
    DefaultAliasRegex("^(default|thinlto-pre-link|thinlto|lto-pre-link|lto)<(O[0123sz])>$");

/// Flag to enable inline deferral during PGO.
static cl::opt<bool> EnablePGOInlineDeferral("enable-npm-pgo-inline-deferral", cl::init(true),
                                             cl::Hidden,
                                             cl::desc("Enable inline deferral during PGO"));

static cl::opt<bool> EnableMemProfiler("enable-mem-prof", cl::init(false), cl::Hidden,
                                       cl::ZeroOrMore, cl::desc("Enable memory profiler"));
...
struct NoOpCGSCCPass : PassInfoMixin<NoOpCGSCCPass> {
    PreservedAnalyses run(LazyCallGraph::SCC &C, CGSCCAnalysisManager &, LazyCallGraph &,
...
class NoOpLoopAnalysis : public AnalysisInfoMixin<NoOpLoopAnalysis> {
  ...
  public:
    struct Result {};
    Result run(Loop &, LoopAnalysisManager &, LoopStandardAnalysisResults &) { return Result(); }
...
PassBuilder::PassBuilder(bool DebugLogging, TargetMachine *TM, PipelineTuningOptions PTO,
                         Optional<PGOOptions> PGOOpt, PassInstrumentationCallbacks *PIC)
...
FunctionPassManager PassBuilder::buildO1FunctionSimplificationPipeline(OptimizationLevel Level,
    ...
    FPM.addPass(createFunctionToLoopPassAdaptor(std::move(LPM1), EnableMSSALoopDependency,
    ...
    FPM.addPass(createFunctionToLoopPassAdaptor(std::move(LPM2), /*UseMemorySSA=*/false,
...
FunctionPassManager PassBuilder::buildFunctionSimplificationPipeline(OptimizationLevel Level,
    ...
    if (PGOOpt && PGOOpt->Action == PGOOptions::IRUse && !Level.isOptimizingForSize())
    ...
    // TODO: Investigate promotion cap for O1.
    LPM1.addPass(LICMPass(PTO.LicmMssaOptCap, PTO.LicmMssaNoAccForPromotionCap));
    LPM1.addPass(SimpleLoopUnswitchPass(/* NonTrivial */ Level == OptimizationLevel::O3));
    LPM2.addPass(LoopIdiomRecognizePass());
    LPM2.addPass(IndVarSimplifyPass());
    ...
    FPM.addPass(createFunctionToLoopPassAdaptor(
        LICMPass(PTO.LicmMssaOptCap, PTO.LicmMssaNoAccForPromotionCap), EnableMSSALoopDependency,
        /*UseBlockFrequencyInfo=*/true, DebugLogging));
    ...
    if (EnableCHR && Level == OptimizationLevel::O3 && PGOOpt &&
        (PGOOpt->Action == PGOOptions::IRUse || PGOOpt->Action == PGOOptions::SampleUse))
...
void PassBuilder::addPGOInstrPasses(ModulePassManager &MPM, PassBuilder::OptimizationLevel Level,
    ...
    FunctionPassManager FPM;
    // Disable header duplication in loop rotation at -Oz.
    FPM.addPass(createFunctionToLoopPassAdaptor(LoopRotatePass(Level != OptimizationLevel::Oz),
...
void PassBuilder::addPGOInstrPassesForO0(ModulePassManager &MPM, bool RunProfileGen, bool IsCS,
...
ModuleInlinerWrapperPass PassBuilder::buildInlinerPipeline(OptimizationLevel Level,
    ...
    ModuleInlinerWrapperPass MIWP(IP, DebugLogging, PerformMandatoryInliningsFirst,
    ...
    MainCGPipeline.addPass(
        createCGSCCToFunctionPassAdaptor(buildFunctionSimplificationPipeline(Level, Phase)));
...
ModulePassManager PassBuilder::buildModuleSimplificationPipeline(OptimizationLevel Level,
    ...
    if (PGOOpt && PGOOpt->PseudoProbeForProfiling && Phase != ThinOrFullLTOPhase::ThinLTOPostLink)
    ...
    bool LoadSampleProfile =
        HasSampleProfile && !(FlattenedProfileUsed && Phase == ThinOrFullLTOPhase::ThinLTOPostLink);
    ...
    if (LoadSampleProfile) {
        ...
        // the debug info.
        MPM.addPass(
            SampleProfileLoaderPass(PGOOpt->ProfileFile, PGOOpt->ProfileRemappingFile, Phase));
        // Cache ProfileSummaryAnalysis once to avoid the potential need to insert
        // RequireAnalysisPass for PSI before subsequent non-module passes.
        MPM.addPass(RequireAnalysisPass<ProfileSummaryAnalysis, Module>());
        ...
        if (Phase != ThinOrFullLTOPhase::ThinLTOPreLink)
            ...
            // removed.
            MPM.addPass(PGOIndirectCallPromotion(Phase == ThinOrFullLTOPhase::ThinLTOPostLink,
                                                 true /* SamplePGO */));
    }
    ...
    if (PGOOpt && Phase != ThinOrFullLTOPhase::ThinLTOPostLink &&
        (PGOOpt->Action == PGOOptions::IRInstr || PGOOpt->Action == PGOOptions::IRUse)) {
...
ModulePassManager PassBuilder::buildModuleOptimizationPipeline(OptimizationLevel Level,
    ...
    if (!LTOPreLink && PGOOpt) {
        if (PGOOpt->CSAction == PGOOptions::CSIRInstr)
            ...
            addPGOInstrPasses(MPM, Level, /* RunProfileGen */ false,
                              /* IsCS */ true, PGOOpt->ProfileFile, PGOOpt->ProfileRemappingFile);
    }
    ...
    OptimizePM.addPass(createFunctionToLoopPassAdaptor(
        LoopRotatePass(Level != OptimizationLevel::Oz, LTOPreLink), EnableMSSALoopDependency,
        /*UseBlockFrequencyInfo=*/false, DebugLogging));
    ...
    OptimizePM.addPass(
        LoopVectorizePass(LoopVectorizeOptions(!PTO.LoopInterleaving, !PTO.LoopVectorization)));
    ...
    if (Level.getSpeedupLevel() > 1 && ExtraVectorizerPasses) {
        ...
        LPM.addPass(SimpleLoopUnswitchPass(/* NonTrivial */ Level == OptimizationLevel::O3));
        OptimizePM.addPass(RequireAnalysisPass<OptimizationRemarkEmitterAnalysis, Function>());
        OptimizePM.addPass(createFunctionToLoopPassAdaptor(std::move(LPM), EnableMSSALoopDependency,
    ...
    OptimizePM.addPass(LoopUnrollPass(LoopUnrollOptions(Level.getSpeedupLevel(),
                                                        /*OnlyWhenForced=*/!PTO.LoopUnrolling,
                                                        PTO.ForgetAllSCEVInLoopUnroll)));
...
ModulePassManager PassBuilder::buildPerModuleDefaultPipeline(OptimizationLevel Level,
    ...
    MPM.addPass(buildModuleSimplificationPipeline(
        Level, LTOPreLink ? ThinOrFullLTOPhase::FullLTOPreLink : ThinOrFullLTOPhase::None));
...
ModulePassManager PassBuilder::buildThinLTOPreLinkDefaultPipeline(OptimizationLevel Level) {
    ...
    // can.
    MPM.addPass(buildModuleSimplificationPipeline(Level, ThinOrFullLTOPhase::ThinLTOPreLink));
...
ModulePassManager PassBuilder::buildLTODefaultPipeline(OptimizationLevel Level,
    ...
    if (PGOOpt && PGOOpt->Action == PGOOptions::SampleUse) {
        ...
        MPM.addPass(SampleProfileLoaderPass(PGOOpt->ProfileFile, PGOOpt->ProfileRemappingFile,
    ...
    // invoke or a call.
    // Run the inliner now.
    MPM.addPass(ModuleInlinerWrapperPass(getInlineParamsFromOptLevel(Level), DebugLogging));
...
ModulePassManager PassBuilder::buildO0DefaultPipeline(OptimizationLevel Level, bool LTOPreLink) {
    assert(Level == OptimizationLevel::O0 && "buildO0DefaultPipeline should only be used with O0");
    ...
    if (PGOOpt && (PGOOpt->Action == PGOOptions::IRInstr || PGOOpt->Action == PGOOptions::IRUse))
        addPGOInstrPassesForO0(MPM,
                               /* RunProfileGen */ (PGOOpt->Action == PGOOptions::IRInstr),
                               /* IsCS */ false, PGOOpt->ProfileFile, PGOOpt->ProfileRemappingFile);
    ...
    if (!LateLoopOptimizationsEPCallbacks.empty()) {
        ...
        if (!LPM.isEmpty()) {
            MPM.addPass(
                createModuleToFunctionPassAdaptor(createFunctionToLoopPassAdaptor(std::move(LPM))));
        }
    }
...
auto parsePassParameters(ParametersParseCallableT &&Parser, StringRef Name, StringRef PassName)
    -> decltype(Parser(StringRef{})) {
    using ParametersT = typename decltype(Parser(StringRef{}))::value_type;

    StringRef Params = Name;
    if (!Params.consume_front(PassName)) {
        assert(false && "unable to strip pass name from parametrized pass specification");
    }
    if (Params.empty())
        return ParametersT{};
...
Expected<MemorySanitizerOptions> parseMSanPassOptions(StringRef Params) {
    ...
    while (!Params.empty()) {
        ...
        if (ParamName == "recover") {
            ...
            return make_error<StringError>(
                formatv("invalid MemorySanitizer pass parameter '{0}' ", ParamName).str(),
                inconvertibleErrorCode());
        }
    }
    return Result;
...
Expected<bool> parseLoopUnswitchOptions(StringRef Params) {
    ...
    while (!Params.empty()) {
        ...
        if (ParamName == "nontrivial") {
            ...
            return make_error<StringError>(
                formatv("invalid LoopUnswitch pass parameter '{0}' ", ParamName).str(),
                inconvertibleErrorCode());
        }
    }
    return Result;
...
Expected<bool> parseMergedLoadStoreMotionOptions(StringRef Params) {
    ...
    while (!Params.empty()) {
        ...
        if (ParamName == "split-footer-bb") {
            ...
            return make_error<StringError>(
                formatv("invalid MergedLoadStoreMotion pass parameter '{0}' ", ParamName).str(),
                inconvertibleErrorCode());
        }
    }
    return Result;
...
static bool startsWithDefaultPipelineAliasPrefix(StringRef Name) {
    return Name.startswith("default") || Name.startswith("thinlto") || Name.startswith("lto");
}
...
template <typename CallbacksT> static bool isModulePassName(StringRef Name, CallbacksT &Callbacks) {
...
template <typename CallbacksT> static bool isCGSCCPassName(StringRef Name, CallbacksT &Callbacks) {
    // Explicitly handle pass manager names.
    if (Name == "cgscc")
        return true;
    if (Name == "function")
...
template <typename CallbacksT> static bool isLoopPassName(StringRef Name, CallbacksT &Callbacks) {
    // Explicitly handle pass manager names.
    if (Name == "loop" || Name == "loop-mssa")
        return true;
...
Optional<std::vector<PassBuilder::PipelineElement>> PassBuilder::parsePipelineText(StringRef Text) {
...
Error PassBuilder::parseModulePass(ModulePassManager &MPM, const PipelineElement &E) {
    ...
    if (startsWithDefaultPipelineAliasPrefix(Name)) {
        ...
        if (L == OptimizationLevel::O0 && Matches[1] != "thinlto" && Matches[1] != "lto") {
...
#define LOOP_PASS(NAME, CREATE_PASS)                                                               \
    if (Name == NAME) {                                                                            \
        MPM.addPass(createModuleToFunctionPassAdaptor(                                             \
            createFunctionToLoopPassAdaptor(CREATE_PASS, false, false, DebugLogging)));            \
        return Error::success();                                                                   \
    }
#define LOOP_PASS_WITH_PARAMS(NAME, CREATE_PASS, PARSER)                                           \
    if (checkParametrizedPassName(Name, NAME)) {                                                   \
...
#include "PassRegistry.def"
    for (auto &C : ModulePipelineParsingCallbacks)
        if (C(Name, MPM, InnerPipeline))
            return Error::success();
    return make_error<StringError>(formatv("unknown module pass '{0}'", Name).str(),
                                   inconvertibleErrorCode());
}

Error PassBuilder::parseCGSCCPass(CGSCCPassManager &CGPM, const PipelineElement &E) {
    auto &Name = E.Name;
    auto &InnerPipeline = E.InnerPipeline;
    ...
    if (!InnerPipeline.empty()) {
        ...
        if (auto MaxRepetitions = parseDevirtPassName(Name)) {
            ...
            if (auto Err = parseCGSCCPassPipeline(NestedCGPM, InnerPipeline))
                return Err;
            CGPM.addPass(createDevirtSCCRepeatedPass(std::move(NestedCGPM), *MaxRepetitions));
            return Error::success();
        }
...
#define CGSCC_ANALYSIS(NAME, CREATE_PASS)                                                          \
    ...
    if (Name == "invalidate<" NAME ">") {                                                          \
        CGPM.addPass(                                                                              \
            InvalidateAnalysisPass<std::remove_reference<decltype(CREATE_PASS)>::type>());         \
        return Error::success();                                                                   \
    }
#define FUNCTION_PASS(NAME, CREATE_PASS)                                                           \
    if (Name == NAME) {                                                                            \
...
#define LOOP_PASS_WITH_PARAMS(NAME, CREATE_PASS, PARSER)                                           \
    if (checkParametrizedPassName(Name, NAME)) {                                                   \
        ...
        if (!Params)                                                                               \
            return Params.takeError();                                                             \
        CGPM.addPass(createCGSCCToFunctionPassAdaptor(createFunctionToLoopPassAdaptor(             \
            CREATE_PASS(Params.get()), false, false, DebugLogging)));                              \
        return Error::success();                                                                   \
    }
#include "PassRegistry.def"
    ...
    for (auto &C : CGSCCPipelineParsingCallbacks)
        if (C(Name, CGPM, InnerPipeline))
            return Error::success();
    return make_error<StringError>(formatv("unknown cgscc pass '{0}'", Name).str(),
                                   inconvertibleErrorCode());
}

Error PassBuilder::parseFunctionPass(FunctionPassManager &FPM, const PipelineElement &E) {
    auto &Name = E.Name;
    auto &InnerPipeline = E.InnerPipeline;
    ...
    if (!InnerPipeline.empty()) {
        ...
        if (Name == "loop" || Name == "loop-mssa") {
            ...
            bool UseBFI =
                llvm::any_of(InnerPipeline, [](auto Pipeline) { return Pipeline.Name == "licm"; });
            FPM.addPass(createFunctionToLoopPassAdaptor(std::move(LPM), UseMemorySSA, UseBFI,
                                                        DebugLogging));
            return Error::success();
        }
        if (auto Count = parseRepeatPassName(Name)) {
...
#define FUNCTION_ANALYSIS(NAME, CREATE_PASS)                                                       \
    if (Name == "require<" NAME ">") {                                                             \
        FPM.addPass(                                                                               \
            RequireAnalysisPass<std::remove_reference<decltype(CREATE_PASS)>::type, Function>());  \
        return Error::success();                                                                   \
    }                                                                                              \
    if (Name == "invalidate<" NAME ">") {                                                          \
        FPM.addPass(InvalidateAnalysisPass<std::remove_reference<decltype(CREATE_PASS)>::type>()); \
        return Error::success();                                                                   \
    }
...
#define LOOP_PASS(NAME, CREATE_PASS)                                                               \
    if (Name == NAME) {                                                                            \
        FPM.addPass(createFunctionToLoopPassAdaptor(CREATE_PASS, false, false, DebugLogging));     \
        return Error::success();                                                                   \
    }
#define LOOP_PASS_WITH_PARAMS(NAME, CREATE_PASS, PARSER)                                           \
    if (checkParametrizedPassName(Name, NAME)) {                                                   \
        ...
        if (!Params)                                                                               \
            return Params.takeError();                                                             \
        FPM.addPass(createFunctionToLoopPassAdaptor(CREATE_PASS(Params.get()), false, false,       \
                                                    DebugLogging));                                \
        return Error::success();                                                                   \
    }
#include "PassRegistry.def"
    ...
    for (auto &C : FunctionPipelineParsingCallbacks)
        if (C(Name, FPM, InnerPipeline))
            return Error::success();
    return make_error<StringError>(formatv("unknown function pass '{0}'", Name).str(),
                                   inconvertibleErrorCode());
}
...
#define LOOP_ANALYSIS(NAME, CREATE_PASS)                                                           \
    if (Name == "require<" NAME ">") {                                                             \
        LPM.addPass(RequireAnalysisPass<std::remove_reference<decltype(CREATE_PASS)>::type, Loop,  \
                                        LoopAnalysisManager, LoopStandardAnalysisResults &,        \
                                        LPMUpdater &>());                                          \
        return Error::success();                                                                   \
    }                                                                                              \
    if (Name == "invalidate<" NAME ">") {                                                          \
        LPM.addPass(InvalidateAnalysisPass<std::remove_reference<decltype(CREATE_PASS)>::type>()); \
        return Error::success();                                                                   \
    }
#include "PassRegistry.def"
...
#define MODULE_ALIAS_ANALYSIS(NAME, CREATE_PASS)                                                   \
    if (Name == NAME) {                                                                            \
        AA.registerModuleAnalysis<std::remove_reference<decltype(CREATE_PASS)>::type>();           \
        return true;                                                                               \
    }
#define FUNCTION_ALIAS_ANALYSIS(NAME, CREATE_PASS)                                                 \
    if (Name == NAME) {                                                                            \
        AA.registerFunctionAnalysis<std::remove_reference<decltype(CREATE_PASS)>::type>();         \
        return true;                                                                               \
    }
#include "PassRegistry.def"
...
Error PassBuilder::parseLoopPassPipeline(LoopPassManager &LPM, ArrayRef<PipelineElement> Pipeline) {
...
void PassBuilder::crossRegisterProxies(LoopAnalysisManager &LAM, FunctionAnalysisManager &FAM,
...
Error PassBuilder::parsePassPipeline(ModulePassManager &MPM, StringRef PipelineText) {
    ...
    if (!Pipeline || Pipeline->empty())
        return make_error<StringError>(formatv("invalid pipeline '{0}'", PipelineText).str(),
                                       inconvertibleErrorCode());
    ...
    if (!isModulePassName(FirstName, ModulePipelineParsingCallbacks)) {
        if (isCGSCCPassName(FirstName, CGSCCPipelineParsingCallbacks)) {
            ...
            return make_error<StringError>(formatv("unknown {0} name '{1}'",
                                                   (InnerPipeline.empty() ? "pass" : "pipeline"),
                                                   FirstName)
                                               .str(),
                                           inconvertibleErrorCode());
        }
...
Error PassBuilder::parsePassPipeline(CGSCCPassManager &CGPM, StringRef PipelineText) {
    auto Pipeline = parsePipelineText(PipelineText);
    if (!Pipeline || Pipeline->empty())
        return make_error<StringError>(formatv("invalid pipeline '{0}'", PipelineText).str(),
                                       inconvertibleErrorCode());
    ...
    if (!isCGSCCPassName(FirstName, CGSCCPipelineParsingCallbacks))
        return make_error<StringError>(
            formatv("unknown cgscc pass '{0}' in pipeline '{1}'", FirstName, PipelineText).str(),
            inconvertibleErrorCode());
...
// FunctionPassManager
Error PassBuilder::parsePassPipeline(FunctionPassManager &FPM, StringRef PipelineText) {
    auto Pipeline = parsePipelineText(PipelineText);
    if (!Pipeline || Pipeline->empty())
        return make_error<StringError>(formatv("invalid pipeline '{0}'", PipelineText).str(),
                                       inconvertibleErrorCode());
    ...
    if (!isFunctionPassName(FirstName, FunctionPipelineParsingCallbacks))
        return make_error<StringError>(
            formatv("unknown function pass '{0}' in pipeline '{1}'", FirstName, PipelineText).str(),
            inconvertibleErrorCode());
...
Error PassBuilder::parsePassPipeline(LoopPassManager &CGPM, StringRef PipelineText) {
    auto Pipeline = parsePipelineText(PipelineText);
    if (!Pipeline || Pipeline->empty())
        return make_error<StringError>(formatv("invalid pipeline '{0}'", PipelineText).str(),
                                       inconvertibleErrorCode());
...
Error PassBuilder::parseAAPipeline(AAManager &AA, StringRef PipelineText) {
    ...
    while (!PipelineText.empty()) {
        ...
        if (!parseAAPassName(AA, Name))
            return make_error<StringError>(formatv("unknown alias analysis name '{0}'", Name).str(),
                                           inconvertibleErrorCode());
    }
...
void PassBuilder::registerParseTopLevelPipelineCallback(
    const std::function<bool(ModulePassManager &, ArrayRef<PipelineElement>, bool DebugLogging)>
        &C) {
