...
static cl::opt<LTOBitcodeEmbedding> EmbedBitcode(
    ...
    cl::values(clEnumValN(LTOBitcodeEmbedding::DoNotEmbed, "none", "Do not embed"),
               clEnumValN(LTOBitcodeEmbedding::EmbedOptimized, "optimized",
                          "Embed after all optimization passes"),
               clEnumValN(LTOBitcodeEmbedding::EmbedPostMergePreOptimized, "post-merge-pre-opt",
                          "Embed post merge, but before optimizations")),
    cl::desc("Embed LLVM bitcode in object files produced by LTO"));

static cl::opt<bool>
    ThinLTOAssumeMerged("thinlto-assume-merged", cl::init(false),
...
Error Config::addSaveTemps(std::string OutputFileName, bool UseInputModulePath) {
    ...
    std::error_code EC;
    ResolutionFile = std::make_unique<raw_fd_ostream>(OutputFileName + "resolution.txt", EC,
...
#include "llvm/Support/Extension.def"
    ...
    for (auto &PluginFN : PassPlugins) {
        ...
        if (!PassPlugin) {
            errs() << "Failed to load passes from '" << PluginFN << "'. Request ignored.\n";
            continue;
        }
...
static std::unique_ptr<TargetMachine> createTargetMachine(const Config &Conf,
    ...
    if (Conf.RelocModel)
        ...
        RelocModel = M.getPICLevel() == PICLevel::NotPIC ? Reloc::Static : Reloc::PIC_;
    ...
    std::unique_ptr<TargetMachine> TM(
        TheTarget->createTargetMachine(TheTriple, Conf.CPU, Features.getString(), Conf.Options,
                                       RelocModel, CodeModel, Conf.CGOptLevel));
    assert(TM && "Failed to create target machine");
    return TM;
}

static cl::opt<bool> EnableSymbolObfuscation("enable-symobfu", cl::init(false), cl::Hidden,
                                             cl::ZeroOrMore, cl::desc("Enable symbol obfuscation"));
...
static void runNewPMPasses(const Config &Conf, Module &Mod, TargetMachine *TM, unsigned OptLevel,
    ...
    if (!Conf.SampleProfile.empty())
        PGOOpt = PGOOptions(Conf.SampleProfile, "", Conf.ProfileRemapping, PGOOptions::SampleUse,
                            PGOOptions::NoCSAction, true);
    else if (Conf.RunCSIRInstr) {
        PGOOpt = PGOOptions("", Conf.CSIRProfile, Conf.ProfileRemapping, PGOOptions::IRUse,
    ...
    if (EnableSymbolObfuscation) {
        MPM.addPass(Pluto::SymbolObfuscation());
    }

    MPM.run(Mod, MAM);
}

static void runNewPMCustomPasses(const Config &Conf, Module &Mod, TargetMachine *TM,
                                 std::string PipelineDesc, std::string AAPipelineDesc,
                                 bool DisableVerify) {
    ...
    if (!AAPipelineDesc.empty())
        if (auto Err = PB.parseAAPipeline(AA, AAPipelineDesc))
            report_fatal_error("unable to parse AA pipeline description '" + AAPipelineDesc +
    ...
    if (auto Err = PB.parsePassPipeline(MPM, PipelineDesc))
        report_fatal_error("unable to parse pass pipeline description '" + PipelineDesc +
                           "': " + toString(std::move(Err)));
...
static void runOldPMPasses(const Config &Conf, Module &Mod, TargetMachine *TM, bool IsThinLTO,
...
bool lto::opt(const Config &Conf, TargetMachine *TM, unsigned Task, Module &Mod, bool IsThinLTO,
              ModuleSummaryIndex *ExportSummary, const ModuleSummaryIndex *ImportSummary,
    ...
    if (EmbedBitcode == LTOBitcodeEmbedding::EmbedPostMergePreOptimized) {
        ...
        // that case.
        if (CmdArgs.empty())
            LLVM_DEBUG(dbgs() << "Post-(Thin)LTO merge bitcode embedding was requested, but "
    ...
    if (!Conf.OptPipeline.empty())
        runNewPMCustomPasses(Conf, Mod, TM, Conf.OptPipeline, Conf.AAPipeline, Conf.DisableVerify);
    else if (Conf.UseNewPM)
        runNewPMPasses(Conf, Mod, TM, Conf.OptLevel, IsThinLTO, ExportSummary, ImportSummary);
    else
        runOldPMPasses(Conf, Mod, TM, IsThinLTO, ExportSummary, ImportSummary);
    return !Conf.PostOptModuleHook || Conf.PostOptModuleHook(Task, Mod);
}

static void codegen(const Config &Conf, TargetMachine *TM, AddStreamFn AddStream, unsigned Task,
    ...
    if (!Conf.DwoDir.empty()) {
        ...
        if (auto EC = llvm::sys::fs::create_directories(Conf.DwoDir))
            report_fatal_error("Failed to create directory " + Conf.DwoDir + ": " + EC.message());
    ...
    CodeGenPasses.add(createImmutableModuleSummaryIndexWrapperPass(&CombinedIndex));
    if (Conf.PreCodeGenPassesHook)
        Conf.PreCodeGenPassesHook(CodeGenPasses);
    if (TM->addPassesToEmitFile(CodeGenPasses, *Stream->OS, DwoOut ? &DwoOut->os() : nullptr,
                                Conf.CGFileType))
...
static void splitCodeGen(const Config &C, TargetMachine *TM, AddStreamFn AddStream,
                         unsigned ParallelCodeGenParallelismLevel, std::unique_ptr<Module> Mod,
                         const ModuleSummaryIndex &CombinedIndex) {
    ThreadPool CodegenThreadPool(heavyweight_hardware_concurrency(ParallelCodeGenParallelismLevel));
            ...
            CodegenThreadPool.async(
                [&](const SmallString<0> &BC, unsigned ThreadId) {
                    ...
                    Expected<std::unique_ptr<Module>> MOrErr = parseBitcodeFile(
                        MemoryBufferRef(StringRef(BC.data(), BC.size()), "ld-temp.o"), Ctx);
                    if (!MOrErr)
...
Error lto::finalizeOptimizationRemarks(std::unique_ptr<ToolOutputFile> DiagOutputFile) {
...
Error lto::backend(const Config &C, AddStreamFn AddStream, unsigned ParallelCodeGenParallelismLevel,
    ...
    if (ParallelCodeGenParallelismLevel == 1) {
        ...
        splitCodeGen(C, TM.get(), AddStream, ParallelCodeGenParallelismLevel, std::move(Mod),
                     CombinedIndex);
    }
    return Error::success();
}
...
Error lto::thinBackend(const Config &Conf, unsigned Task, AddStreamFn AddStream, Module &Mod,
                       const ModuleSummaryIndex &CombinedIndex,
    ...
    auto DiagFileOrErr = lto::setupLLVMOptimizationRemarks(
        Mod.getContext(), Conf.RemarksFilename, Conf.RemarksPasses, Conf.RemarksFormat,
        Conf.RemarksWithHotness, Conf.RemarksHotnessThreshold, Task);
    if (!DiagFileOrErr)
    ...
    auto OptimizeAndCodegen = [&](Module &Mod, TargetMachine *TM,
        ...
        if (!opt(Conf, TM, Task, Mod, /*IsThinLTO=*/true,
                 /*ExportSummary=*/nullptr, /*ImportSummary=*/&CombinedIndex, CmdArgs))
            return finalizeOptimizationRemarks(std::move(DiagnosticOutputFile));
    ...
    if (Conf.PostInternalizeModuleHook && !Conf.PostInternalizeModuleHook(Task, Mod))
        return finalizeOptimizationRemarks(std::move(DiagnosticOutputFile));
...
Expected<BitcodeModule> lto::findThinLTOModule(MemoryBufferRef MBRef) {
    ...
    return make_error<StringError>("Could not find module summary", inconvertibleErrorCode());
}
...
bool lto::loadReferencedModules(
    ...
    std::vector<std::unique_ptr<llvm::MemoryBuffer>> &OwnedImportsLifetimeManager) {
    if (ThinLTOAssumeMerged)
        return true;
    ...
    for (auto &I : ImportList) {
        ...
        if (!BMOrErr) {
            handleAllErrors(BMOrErr.takeError(), [&](ErrorInfoBase &EIB) {
                errs() << "Error loading imported file '" << I.first() << "': " << EIB.message()
                       << '\n';
            });
            return false;
        }
