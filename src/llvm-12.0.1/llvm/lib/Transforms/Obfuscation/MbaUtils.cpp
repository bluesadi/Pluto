...
Value *MbaUtils::insertLinearMBA(int64_t *params, Instruction *insertBefore) {
    ...
    if (isa<BinaryOperator>(insertBefore) && insertBefore->getNumOperands() == 2) {
        ...
        GlobalVariable *xPtr = new GlobalVariable(M, type, false, GlobalValue::PrivateLinkage,
                                                  ConstantInt::get(type, randX), "x");
        GlobalVariable *yPtr = new GlobalVariable(M, type, false, GlobalValue::PrivateLinkage,
